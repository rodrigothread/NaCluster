...
/**
 * The DataSourceManager is the component that creates CoDIMS dataSources. When creating a query
 * execution plan, the QEPFactory calls the DataSourceManager to creates existing dataSources
 * in the plan. The dataSources are put in the BlackBoard for further use.
 * 
 * Added by Othman :
 * - The DataSourceManager is a singleton.
 * - The DataSourceManager creates each dataSource using the reflection. 
 * - Javadoc translated.
 * 
...
public class DataSourceManager {
     ...
     * Log4j logger.
     */
    protected Logger logger = Logger.getLogger(DataSourceManager.class.getName());
    ...
    /**
     * Create the dataSources according to the informations encapsulated in the OpNode structure.
     * 
     * @param opNode structure containing node indormations.
     * @throws DataSourceException
     * @throws OperatorInitializationException
     */
    public void createDataSources(OpNode opNode) 
    	throws DataSourceException, OperatorInitializationException {
    	...
    	String className = (String) catalogManager.getSingleObject("dataSource","className",
    			"name='" + dataSourceName + "'");
    	...
    	try {
    		...
    		// Second parameter of the constructor is a TupleMetadata object set to null.
		...
		// Put the datasource in the BlackBoard and reference it with its timeStamp.
		String key = opNode.getOpTimeStamp();
		dataSources.put(key, ds);
	} catch (Exception ex) {
		throw new OperatorInitializationException("DataSource initialization " +
