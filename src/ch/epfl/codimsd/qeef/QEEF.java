...
public class QEEF {
     ...
     * Log4j logger.
     */
    private static Logger logger = Logger.getLogger(QEEF.class.getName());
    ...
    public RequestResult execute() throws Exception {
    	...
    	Operator rootOp = (Operator) planOps.get(new Integer(1));
...
//        	pa.log("Opening operators : ", 	System.currentTimeMillis()-beginOpeningOperatorsTime);
        	logger.info("Operators opened.");
                System.out.println("Operators opened.");
...
//        pa.log("FinishingTime : ", System.currentTimeMillis()-finishingTime);
	...
	public DataHandler executeRemote() throws RemoteException {
    	...
    	try {
    		...
    		long beginOpeningOperatorsTime = System.currentTimeMillis();
        	open(rootOp);
        	pa.log("Opening operators : ", 
    				System.currentTimeMillis()-beginOpeningOperatorsTime);
    		...
    		pa.log("FinishingTime : ", 
    				System.currentTimeMillis()-finishingTime);
    		pa.finalizeLogging();
    ...
    public void open(int idOp) throws Exception {
        ...
        Operator op;
        op = (Operator) this.planOps.get(new Integer(idOp));

        if (op == null) {
            logger.warn("QEEF error: Operator not defined " + idOp);
            throw new Exception("Operator " + idOp
                    + " is not defined in this execution plan.");
        }
    ...
    /**
     * Obtem um operador do plano em execu��o na m�quina.
     * 
     * @param opId Identificador do operador.
     */
    protected Operator getPlanOperator(int opId) {
        return (Operator) planOps.get(new Integer(opId));
    }
    ...
    public void init(String qepString) throws InitializationException, OperatorException, CatalogException, OperatorInitializationException {

                OperatorFactoryManager operatorFactoryManager = new OperatorFactoryManager();
                WEBBPlanManager discoveryPlanManager = new WEBBPlanManager(operatorFactoryManager);
