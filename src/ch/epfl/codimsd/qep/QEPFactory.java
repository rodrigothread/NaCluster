...
public class QEPFactory {
	 ...
	 */
	public static String getQEP(int requestType, int templateID) throws RequestTypeNotFoundException, CatalogException {
		...
		String query = "SELECT PlanTemplate.path FROM PlanTemplate, RequestTypeTemplate WHERE " +
				...
				"RequestTypeTemplate.idTemplate =  PlanTemplate.idTemplate AND " +
				"PlanTemplate.templateType = " + templateID;
			...
			if (opNode.getType() != null) {
				...
				if (opNode.getType().equalsIgnoreCase("Scan")) {
					...
					try {
						...
						throw new QEPInitializationException("Error creating the " +
								"datasource : " + ex.getMessage());
					}
				}
			}
